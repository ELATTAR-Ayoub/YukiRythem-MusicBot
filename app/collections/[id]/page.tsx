...
export default function Page({ params }: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      dispatch(setMusicState(data.music));
      setLoading(false);
    };
    fetchData();
  }, [params.id]);
  ...
  return (
    <section
      className={` ${styles.flexCenter} flex-col relative overflow-hidden w-full`}
    >
      {loading ? (
      ...
      <div
        className={` ${styles.flexStart} flex-col relative text-secondary-color bg-primary-color-4  dark:text-primary-color-4 dark:bg-secondary-color w-full p-8 gap-4`}
      >
        <div className={` ${styles.flexStart} w-full flex-col gap-2 mb-2`}>
          <h1 className={` ${styles.h2Section} my-0 text-left`}>
            {thisCollection.title}
          </h1>
          <p className={` text-base md:text-lg w-full text-left `}>
            {thisCollection.desc}
          </p>
        </div>
        <div className={` ${styles.flexStart} gap-2 `}>
          {thisCollection.tags &&
            thisCollection.tags.map((tag: any, index: number) => (
              <div
                key={index}
        ...
        <div className={` flex justify-center items-center w-full gap-2`}>
          <div className="grid grid-cols-2 content-center btn-rounded-primary">
            <button
              onClick={() => handleLikeCollection(thisCollection)}
              aria-label="open_music_list"
            >
              {user.lovedCollections.includes(thisCollection.UID_Col) ? (
                <SolidSvg
                  width={"24px"}
                  height={"24px"}
                  color={"#ED493E"}
          ...
          <p className={`  ${styles.Paragraph_sm} text-left `}>
            {" "}
            <Link
              className="underline ml-2 hover:text-primary-color-77 dark:hover:text-primary-color-53 "
              href={`/profile/${thisCollection.ownerID}`}
            >
              Made by {thisCollection.ownerUserName}
            </Link>{" "}
          </p>
        </div>
        ...
        <div className={` ${styles.flexStart} w-full gap-2`}>
          <p className={`  ${styles.Paragraph_sm} text-left `}>
            {" "}
            {formatDate(thisCollection.date)} - Collection length:{" "}
            {formatTime(thisCollection.collectionLengthSec)}{" "}
          </p>
        </div>
      </div>
      ...
      <div
        id="player"
        className={` ${styles.flexBetween} lg:justify-end flex-col gap-[20px] relative bg-primary-color-4 dark:bg-secondary-color overflow-hidden  h-full w-full `}
      >
        <PlayerUI mode="collection" />
        <div
          className={` ${styles.flexBetween} flex-col bg-secondary-color dark:bg-primary-color-4 p-8  rounded-t-[35px] w-full h-[300px]`}
        >
          <div className=" w-full sm:max-w-[675px] lg:max-w-[800px] xl:max-w-[1014px]">
            {
              <NativeVideo
                videoId={musicState[current] ? musicState[current].ID : ""}
              />
            }
          </div>
        </div>
      ...
      <div
        id="music_list_popup"
        className="fixed bottom-0 h-0 overflow-hidden w-full sm:max-w-[675px] lg:max-w-[800px] xl:max-w-[1014px] rounded-t-lg z-30 transition-all duration-300"
      >
